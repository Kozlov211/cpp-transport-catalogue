...
void SerialHandler::SerializationTransportCatalogue(const TransportCatalogue& transport_catalogue) {
    SerializationStops(transport_catalogue.GetDequeStops());
    SerializationDistanceBetweenStops(transport_catalogue.GetDistanceBetweenStops());
    SerializationBuses(transport_catalogue.GetDequeBuses());
}

void SerialHandler::SerializationStops(const std::deque<Stop>& stops) {
    for (auto& stop : stops) {
        ...
        coordinate_proto->set_lat(stop.сoordinates.lat);
        coordinate_proto->set_lng(stop.сoordinates.lng);
        stop_proto->set_index(stop.index);
    }
}

void SerialHandler::SerializationBuses(const std::deque<Bus>& buses) {
    for (auto& bus : buses) {
        ...
        for (size_t i = 0; i < bus.route.size(); ++i) {
            bus_proto->add_route(bus.route[i]->name);
        }
    }
}

void SerialHandler::SerializationDistanceBetweenStops(const std::unordered_map<std::pair<Stop*, Stop*>, size_t, detail::Hash>& distance_between_stops_) {
...
void SerialHandler::SerializationTransoprtRouter(const TransportRouter& transport_router) {
...
void SerialHandler::SerializationStopAsPairNumber(const std::unordered_map<Stop*, StopPairVertexId>& stop_as_pair_number) {
...
void SerialHandler::SerializationEdgeidToType(const std::unordered_map<graph::EdgeId, EdgeInfo>& edge_id_to_type) {
    ...
    for (const auto& [id, info] : edge_id_to_type) {
        ...
        if (info.index() == 0) {
            ...
            const domain::WaitEdgeInfo& wait_edge = get<domain::WaitEdgeInfo>(info);
            ...
            *edge_info_proto.mutable_wait_edge_info() = move(wait_edge_proto);
            ...
            const domain::BusEdgeInfo& bus_edge = get<domain::BusEdgeInfo>(info);
...
TransportRouter SerialHandler::GetTransportRouter(const TransportCatalogue& transport_catalogue) {
    TransportRouter transport_router;
    transport_router.SetSettings(DeserializeRoutingSettings());
...
void SerialHandler::DeserializeStops(transport_catalogue::TransportCatalogue& transport_catalogue) {
    for (size_t i = 0; i < transport_catalogue_proto_.stops_size(); ++i) {
        ...
        stop.сoordinates = {stop_proto.coordinates().lat(), stop_proto.coordinates().lng()};
        stop.index = stop_proto.index();
        transport_catalogue.AppendStop(move(stop));
    }
}
...
void SerialHandler::DeserializeBuses(transport_catalogue::TransportCatalogue& transport_catalogue) {
    for (size_t i = 0; i < transport_catalogue_proto_.distance_between_stops_size(); ++i) {
        ...
        transport_catalogue.AppendDistanceToStop(transport_catalogue.GetStop(distance_between_stops_proto.stop_from()),
                                                  transport_catalogue.GetStop(distance_between_stops_proto.stop_to()),
...
void SerialHandler::DeserializeDistanceBetweenStops(transport_catalogue::TransportCatalogue& transport_catalogue) {
    for (size_t i = 0; i < transport_catalogue_proto_.buses_size(); ++i) {
        ...
        bus.is_circle = bus_proto.is_circle();
        for (size_t j = 0; j < bus_proto.route_size(); ++j) {
            bus.route.push_back(transport_catalogue.GetStop(bus_proto.route(j)));
        }
        transport_catalogue.AppendBus(move(bus));
    }
}
...
RoutingSettings SerialHandler::DeserializeRoutingSettings() {
...
std::unordered_map<domain::Stop*, StopPairVertexId> SerialHandler::DeserializeStopAsPairNumber(const TransportCatalogue& transport_catalogue) {
    const auto& stop_as_pair_number_proto = transport_catalogue_proto_.transport_router().stop_id_to_pair_vertex_id();
    std::unordered_map<domain::Stop*, StopPairVertexId> stop_as_pair_number(stop_as_pair_number_proto.size());
    for (const auto& [stop, vertex] : stop_as_pair_number_proto) {
        stop_as_pair_number[transport_catalogue.GetStop(stop)] = {vertex.bus_wait_begin(), vertex.bus_wait_end()};
    }
    return stop_as_pair_number;
}

std::unordered_map<graph::EdgeId, EdgeInfo> SerialHandler::DeserializeEdgeidToType(const TransportCatalogue& transport_catalogue) {
    ...
    for (const auto& [id, info] : edge_id_to_type_proto) {
        if (info.has_wait_edge_info()) {
            ...
            domain::WaitEdgeInfo wait_edge = {transport_catalogue.GetStop(wait_edge_proto.stop_name())->name, wait_edge_proto.minutes()};
            edge_id_to_type[id] = move(wait_edge);

        } else {
            const transport_router_proto::BusEdgeInfo& bus_edge_proto = info.bus_edge_info();
            domain::BusEdgeInfo bus_edge = {transport_catalogue.GetBus(bus_edge_proto.bus_name())->name, bus_edge_proto.span_count(), bus_edge_proto.time()};
